...
app.use(cookieSession({ name: 'jestri_final_fix', keys: ['JESTRI_SECURE'], maxAge: 24 * 60 * 60 * 1000 }));
...
app.get('/', (req, res) => {
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .search-bar { width: 100%; padding: 12px; border-radius: 12px; border: none; background: #1e293b; color: white; outline: none; }
        ...
        .nav-head { font-size: 0.8rem; font-weight: 800; color: #38bdf8; margin: 25px 0 10px 5px; }
        ...
        .btn-buy { width: 100%; padding: 10px; border: none; border-radius: 10px; background: #0f172a; color: white; font-weight: 800; font-size: 0.7rem; cursor: pointer; }

        .empty-msg { grid-column: 1/-1; text-align: center; padding: 50px 20px; color: #94a3b8; font-weight: 600; }

        .float-box { position: fixed; bottom: 20px; right: 20px; display: flex; flex-direction: column; gap: 12px; z-index: 4000; }
        .float-btn { width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; text-decoration: none; font-size: 1.5rem; box-shadow: 0 5px 15px rgba(0,0,0,0.2); }
    ...
    <div class="sidebar" id="sb">
        <h2 style="color:#0f172a;">JESTRI MENU</h2>
        ...
        <a href="https://link.dana.id/qr/0895327806441" style="display:block; margin-top:30px; background:#fbbf24; color:black; padding:12px; border-radius:12px; text-align:center; text-decoration:none; font-weight:800;">DONASI</a>
    </div>

    <div class="top-nav">
        <i class="fa-solid fa-bars-staggered" onclick="tog()" style="font-size:1.4rem;"></i>
        <div class="logo">E-BOOK JESTRI</div>
        <div onclick="openCart()" style="position:relative;"><i class="fa-solid fa-cart-shopping" style="font-size:1.4rem;"></i><span id="cc" style="position:absolute; top:-8px; right:-10px; background:#ef4444; color:white; font-size:0.65rem; padding:2px 6px; border-radius:50%; font-weight:800;">0</span></div>
    </div>

    <div class="search-row"><input type="text" id="sr" class="search-bar" placeholder="Cari judul atau penulis..." oninput="cari()"></div>
    ...
    <div id="modal"><div style="background:white; width:100%; max-width:380px; border-radius:24px; padding:25px;">
        ...
        <input type="file" id="fBukti" style="margin-bottom:20px; width:100%;">
        <button onclick="checkout()" id="btnC" style="width:100%; padding:15px; background:#10b981; color:white; border:none; border-radius:12px; font-weight:800;">KONFIRMASI BAYAR</button>
    ...
    <script>
        ...
        function render(data){
            ...
            if(data.length === 0){
                g.innerHTML = \`<div class="empty-msg">Buku dengan genre \${curG} belum ada</div>\`;
                return;
            }
            g.innerHTML = data.map(x => \`
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div style="font-size:0.65rem; color:#64748b; margin-bottom:5px;">\${x.penulis}</div>
        ...
        function cari(){
            ...
            const filt = allB.filter(b => b.judul.toLowerCase().includes(k) || b.penulis.toLowerCase().includes(k));
            render(filt);
        }
        ...
        async function checkout(){
            ...
            document.getElementById('modal').innerHTML = '<div style="background:white;padding:30px;border-radius:24px;text-align:center;"><h3>Berhasil!</h3><p>Pesanan sedang diverifikasi.</p><div id="dl-box">⏳</div></div>';
...
// --- 2. LOGIN ADMIN (RESIZEABLE & STABIL) ---
app.get('/login', (req, res) => {
    res.send(`<body style="background:#0f172a; margin:0; height:100dvh; display:flex; align-items:center; justify-content:center; overflow:hidden; touch-action:none; position:fixed; width:100%;">
    <form action="/login" method="POST" style="background:white; padding:40px 20px; border-radius:24px; width:90%; max-width:350px; text-align:center; box-sizing:border-box;">
        <h2 style="margin:0 0 20px 0; font-family:sans-serif; color:#0f172a;">ADMIN LOGIN</h2>
        <input name="pw" type="password" placeholder="Passcode" autofocus style="width:100%; padding:15px; border:1px solid #ddd; border-radius:12px; text-align:center; font-size:1.1rem; box-sizing:border-box;">
        <button style="width:100%; padding:15px; background:#38bdf8; color:white; border:none; border-radius:12px; font-weight:bold; margin-top:20px; font-size:1rem;">MASUK</button>
    </form></body>`);
});
...
app.get('/admin', async (req, res) => {
    ...
    <style>body{font-family:sans-serif; background:#f1f5f9; padding:15px; margin:0;} .box{background:white; padding:20px; border-radius:18px; margin-bottom:15px;} input, select{width:100%; padding:14px; margin:8px 0; border:1px solid #ddd; border-radius:12px; box-sizing:border-box;}</style></head><body>
    ...
    <div class="box">
        ...
        <button onclick="addB()" id="btnS" style="width:100%; padding:15px; background:#0f172a; color:white; border:none; border-radius:12px; font-weight:bold;">SIMPAN BUKU</button>
    </div>
    <h3>Katalog (${b.length})</h3>
    <div class="box">${b.map(x=>`<div style="display:flex; justify-content:space-between; padding:10px 0; border-bottom:1px solid #eee;"><span>\${x.judul}</span><button onclick="delB('\${x._id}')" style="color:red; border:none; background:none; font-weight:bold;">HAPUS</button></div>`).join('')}</div>
    <script>
        async function addB(){
