...
app.get('/', (req, res) => {
    ...
    <style>
        ...
        /* SIDEBAR OPTIMIZATION: Added overflow-y-auto and custom scrollbar */
        ...
        .book-card:hover { transform: translateY(-5px); border-color: #007AFF; box-shadow: 0 10px 30px rgba(0,122,255,0.1); }
    </style></head><body>
    ...
    <aside id="sb" class="sidebar fixed top-0 left-0 h-full w-80 z-[120] -translate-x-full p-10 flex flex-col">
        <div class="flex justify-between items-center mb-10">
            <span class="text-[10px] font-black tracking-widest text-gray-500 uppercase">Kategori</span>
            <i class="fa-solid fa-xmark cursor-pointer hover:rotate-90 transition" onclick="togSide(false)"></i>
        </div>
        <div class="flex-grow space-y-2">
            <button onclick="setFilter('Semua')" class="w-full text-left p-4 rounded-xl bg-white/5 font-bold hover:bg-blue-600 transition">Semua</button>
            ${LIST_GENRE.map(g => `<button onclick="setFilter('${g}')" class="w-full text-left p-4 rounded-xl text-gray-400 text-sm hover:text-white transition hover:bg-white/5">${g}</button>`).join('')}
        </div>
        <div class="pt-10 mt-10 border-t border-white/5 text-center">
            ...
            <a href="https://saweria.co/jesssstri" class="block w-full p-4 rounded-xl bg-amber-500/10 text-amber-500 border border-amber-500/20 font-black text-[10px] tracking-widest uppercase hover:bg-amber-500 hover:text-white transition">Dukung Kami</a>
        </div>
    </aside>

    <main class="max-w-7xl mx-auto p-8 py-20 min-h-screen">
        <header class="mb-16">
            <h2 id="vTitle" class="text-3xl md:text-5xl font-black mb-2 tracking-tighter leading-tight">Investasi Literasi Digital.</h2>
            <p class="text-gray-600 text-[10px] font-bold tracking-[0.4em] uppercase italic">The Exclusive Collection 2026</p>
        </header>
        <div id="grid" class="grid grid-cols-2 lg:grid-cols-4 gap-6">
    ...
    <footer class="p-16 border-t border-white/5 text-center bg-[#0d0d0d]">
        ...
        <p class="text-gray-700 text-[10px] font-bold tracking-widest uppercase mb-4 italic">© 2026 E-BOOK JESTRI</p>
        <a href="https://saweria.co/jesssstri" class="text-blue-500 text-[10px] font-black uppercase tracking-widest border border-blue-500/20 px-8 py-2 rounded-full hover:bg-blue-500 hover:text-white transition">Donate</a>
    </footer>
    ...
    <div id="md" class="fixed inset-0 z-[200] bg-black/95 hidden items-center justify-center p-4">
        <div class="bg-[#111] w-full max-w-lg rounded-[2.5rem] p-8 border border-white/5 max-h-[90vh] overflow-y-auto">
            <h3 class="text-xl font-black mb-6 text-center italic">Payment Details.</h3>
            ...
            <div class="flex justify-between font-black text-xl mb-8 px-2">
                <span class="text-gray-600 uppercase text-xs self-center">Total Investasi</span>
                <span id="cartTotal" class="text-blue-500">Rp 0</span>
            </div>
            <input type="file" id="fi" class="hidden" onchange="document.getElementById('ftxt').innerText='✅ Bukti Diunggah'">
            <label for="fi" class="block border border-dashed border-white/10 p-5 rounded-2xl text-center cursor-pointer mb-6 text-xs text-gray-500 font-bold uppercase hover:border-blue-500 transition">Pilih Bukti Transfer</label>
            <button onclick="prosesBayar()" id="btnBayar" class="w-full bg-blue-600 py-5 rounded-xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-blue-500 transition">Konfirmasi Sekarang</button>
    ...
    <script>
            ...
            grid.innerHTML = data.map(b => \`
                <div class="book-card">
                    ...
                    <div class="p-4">
                        ...
                        <h4 class="font-bold text-[12px] h-8 line-clamp-2 mb-4 leading-tight">\${b.judul}</h4>
                        <div class="flex justify-between items-center pt-2 border-t border-white/5">
                            <span class="font-black text-sm">Rp \${b.investasi.toLocaleString()}</span>
        ...
        function togCart(st) {
            if(st && !cart.length) return alert("Pilih koleksi.");
        ...
        async function prosesBayar() {
            ...
            try {
                ...
                alert("Pesanan terkirim!"); location.reload();
...
// --- 3. THE ADMIN SYSTEM (LOCAL STORAGE AUTH) ---
app.get('/login', (req, res) => {
    ...
    <div style="text-align:center; background:#111; padding:50px; border-radius:30px; border:1px solid #222; width:300px;">
        <h2 style="font-weight:900; margin-bottom:30px; text-transform:uppercase; letter-spacing:2px;">Elite Access</h2>
        <input id="pw" type="password" placeholder="Passcode" style="padding:15px; border-radius:15px; border:1px solid #333; background:#000; color:white; text-align:center; width:100%; margin-bottom:20px; outline:none; font-weight:bold;">
        <button onclick="login()" style="background:#007AFF; color:white; border:none; padding:15px; border-radius:15px; font-weight:900; cursor:pointer; width:100%; text-transform:uppercase;">Unlock System</button>
    </div>
    <script>
        async function login(){
            const pw = document.getElementById('pw').value;
            // FIX: Mengirim data via API Fetch agar tidak ada "Cannot POST /login"
...
app.get('/admin', (req, res) => {
    ...
    <body class="bg-[#0A0A0A] text-white p-10">
        ...
        <div class="max-w-6xl mx-auto flex justify-between items-center mb-10 border-b border-white/5 pb-5">
            <h1 class="text-xl font-black italic uppercase">Control <span class="text-blue-500">Center</span></h1>
            <div class="flex gap-4 items-center">
                <a href="/" class="text-[10px] font-bold uppercase text-gray-500">Site</a>
                <button onclick="localStorage.removeItem('jestri_key'); location.href='/login';" class="text-[10px] font-bold text-red-500 uppercase">Logout</button>
            </div>
        </div>
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div class="bg-[#111] p-8 rounded-3xl border border-white/5 h-fit shadow-2xl">
                <h3 class="text-[10px] font-black uppercase text-blue-500 mb-8 tracking-widest italic underline">Tambah Produk</h3>
                <div class="space-y-4">
                    <input id="j" placeholder="Judul Buku" class="w-full p-4 bg-black border border-white/10 rounded-xl text-sm outline-none focus:border-blue-500 transition">
                    <input id="h" type="number" placeholder="Harga (Nominal)" class="w-full p-4 bg-black border border-white/10 rounded-xl text-sm outline-none focus:border-blue-500 transition">
                    ...
                    <button onclick="saveB()" id="sb" class="w-full bg-blue-600 py-4 rounded-xl font-black text-xs mt-4 hover:bg-blue-500 transition uppercase">Terbitkan</button>
                </div>
            </div>
        ...
        <script>
            async function load(){
                ...
                <div class="bg-[#111] p-6 rounded-3xl border border-white/5 flex flex-col md:flex-row gap-6 items-center">
                    <img src="\${x.bukti}" class="w-full md:w-32 h-44 object-cover rounded-2xl border border-white/10 shadow-lg">
                    <div class="flex-grow">
                        <h4 class="text-xl font-black text-blue-500 italic tracking-tighter">Rp \${x.total.toLocaleString()}</h4>
                        <p class="text-[10px] text-gray-400 mb-6 uppercase tracking-widest font-bold">\${x.items.map(i=>i.judul).join(' • ')}</p>
                        <div class="flex gap-2">
                            <input id="l-\${x._id}" placeholder="Link Akses E-Book" class="flex-grow p-4 bg-black border border-blue-900/30 rounded-xl text-xs outline-none focus:border-blue-500">
                            <button onclick="acc('\${x._id}')" class="bg-blue-600 px-6 rounded-xl font-black text-[10px] uppercase">Verifikasi</button>
                        </div>
                    </div>
                </div>\`).join('') || '<div class="text-center py-20 text-gray-700 italic font-black uppercase tracking-widest">Belum ada pesanan</div>';
            }
            async function saveB(){
                const f = document.getElementById('cv').files[0]; if(!f) return alert("Pilih cover!");
                const btn = document.getElementById('sb'); btn.disabled = true; btn.innerText = "UPDATING...";
                const fd = new FormData(); fd.append('file', f); fd.append('upload_preset', 'ml_default');
                const up = await (await fetch('https://api.cloudinary.com/v1_1/dxtp7vsqy/image/upload',{method:'POST',body:fd})).json();
            ...
            async function acc(id){
                const link = document.getElementById('l-'+id).value; if(!link) return alert("Isi link!");
                await fetch('/core/approve/'+id, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({link}) });
                alert("Terselesaikan!"); location.reload();
            }
            load();
        </script>
    </body></html>`);
