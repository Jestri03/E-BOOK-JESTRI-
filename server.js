...
// --- 2. THE VISIONARY FRONTEND (MODERN & MINIMALIST) ---
app.get('/', (req, res) => {
    ...
    <style>
        ...
        .sidebar { transition: 0.6s transform cubic-bezier(0.16, 1, 0.3, 1); background: #0F0F11; }
        ...
        .qris-box { background: white; padding: 15px; border-radius: 20px; display: inline-block; box-shadow: 0 10px 30px rgba(255,255,255,0.1); }
    </style></head><body>
    ...
    <aside id="sb" class="sidebar fixed top-0 left-0 h-full w-80 z-[120] -translate-x-full p-10 flex flex-col shadow-2xl">
        <div class="flex justify-between items-center mb-16">
            <span class="text-[10px] font-black tracking-[0.4em] text-gray-500">NAVIGATION</span>
    ...
    <main class="max-w-7xl mx-auto p-8 py-20 min-h-screen">
        <header class="mb-20">
            <h2 id="vTitle" class="text-5xl font-black mb-4 tracking-tighter">Investasi Literasi Digital.</h2>
            <p class="text-gray-500 uppercase text-[10px] tracking-[0.4em] font-black">Professional & Immersive Marketplace</p>
        </header>
    ...
    <footer class="p-20 border-t border-white/5 text-center">
        ...
        <p class="text-gray-700 text-[10px] font-black tracking-widest uppercase mb-4">© 2026 E-BOOK JESTRI. All Rights Reserved.</p>
        <a href="https://saweria.co/jesssstri" class="text-blue-500 text-[10px] font-black uppercase tracking-widest border border-blue-500/20 px-8 py-2 rounded-full">Donate</a>
    </footer>
    ...
    <div id="md" class="fixed inset-0 z-[200] bg-black/95 hidden items-center justify-center p-4">
        <div class="bg-[#111] w-full max-w-lg rounded-[3rem] p-10 border border-white/5 shadow-2xl max-h-[90vh] overflow-y-auto">
            <h3 class="text-2xl font-black mb-8 italic text-center">Checkout VIP.</h3>
            <div id="cartList" class="space-y-4 mb-10"></div>
            
            <div class="bg-white/5 p-8 rounded-[2.5rem] border border-white/5 text-center mb-8">
                <p class="text-[9px] font-black text-blue-500 mb-6 tracking-widest uppercase italic">Scan QRIS / Transfer DANA</p>
                <div class="qris-box mb-6"><img src="https://i.ibb.co.com/jP4fBvY8/Screenshot-20250304-104953-CIMB-OCTO.jpg" class="w-44 h-auto" alt="QRIS"></div>
                <div class="text-lg font-black text-white">0895 3278 06441</div>
            </div>

            <div class="flex justify-between font-black text-2xl mb-10 px-2">
            ...
            <label for="fi" class="block border-2 border-dashed border-white/10 p-6 rounded-3xl text-center cursor-pointer hover:border-blue-500 mb-6">
                <span id="ftxt" class="text-xs font-bold text-gray-500 uppercase tracking-widest">Upload Bukti Transfer</span>
            </label>

            <button onclick="prosesBayar()" id="btnBayar" class="w-full bg-blue-600 py-6 rounded-2xl font-black text-xs tracking-widest uppercase shadow-xl hover:bg-blue-500">Konfirmasi Investasi</button>
            <button onclick="togCart(false)" class="w-full mt-6 text-gray-700 text-[10px] font-bold uppercase tracking-widest">Batal</button>
        </div>
    </div>
    ...
    <script>
        ...
        function togCart(st) {
            ...
            if(st) {
                document.getElementById('cartList').innerHTML = cart.map(x => \`<div class="flex justify-between font-bold text-xs"><span>\${x.judul}</span><span class="text-blue-500">Rp \${x.investasi.toLocaleString()}</span></div>\`).join('');
        ...
        async function prosesBayar() {
            ...
            try {
                ...
                alert("✅ Sukses! Admin akan memproses pesanan Anda segera.");
                location.reload();
...
app.get('/login', (req, res) => {
    ...
    <div style="text-align:center; background:#111; padding:50px; border-radius:40px; border:1px solid #222; width:320px;">
        <h2 style="color:white; font-weight:900; letter-spacing:-1px; margin-bottom:30px;">ELITE ACCESS</h2>
        <input id="pw" type="password" placeholder="Passcode" style="padding:18px; border-radius:20px; border:1px solid #333; background:#000; color:white; width:100%; text-align:center; margin-bottom:20px; outline:none;">
        <button onclick="login()" style="background:#007AFF; color:white; border:none; padding:18px; border-radius:20px; font-weight:900; width:100%; cursor:pointer;">UNLOCK SYSTEM</button>
    </div>
    <script>
    ...
    <body class="bg-[#0A0A0A] text-white p-10">
        ...
        <div class="max-w-6xl mx-auto flex justify-between items-center mb-16">
            ...
            <div class="flex gap-4">
                <a href="/" class="text-xs font-bold text-gray-500 uppercase">Site</a>
                <button onclick="localStorage.removeItem('jestri_token'); location.href='/login';" class="text-xs font-bold text-red-500 uppercase">Logout</button>
            </div>
        </div>
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div class="bg-[#111] p-8 rounded-[2.5rem] border border-white/5 h-fit">
                <h3 class="text-[10px] font-black uppercase text-blue-500 mb-8 tracking-widest">Add Literatur</h3>
                <div class="space-y-4">
                    <input id="j" placeholder="Judul" class="w-full p-4 bg-black border border-white/10 rounded-2xl text-sm outline-none">
                    <input id="p" placeholder="Penulis" class="w-full p-4 bg-black border border-white/10 rounded-2xl text-sm outline-none">
                    <input id="h" type="number" placeholder="Investasi" class="w-full p-4 bg-black border border-white/10 rounded-2xl text-sm outline-none">
                    ...
                    <button onclick="saveB()" id="sb" class="w-full bg-blue-600 py-4 rounded-2xl font-black text-xs uppercase shadow-xl mt-4">Publish</button>
                </div>
            </div>
            <div id="orderList" class="lg:col-span-2 space-y-8">
                </div>
        </div>
        <script>
            async function load(){
                ...
                <div class="bg-[#111] p-8 rounded-[3rem] border border-white/5 flex flex-col md:flex-row gap-8">
                    <a href="\${x.bukti}" target="_blank" class="w-full md:w-52 h-72 block flex-shrink-0"><img src="\${x.bukti}" class="w-full h-full object-cover rounded-3xl border border-white/10"></a>
                    <div class="flex-grow">
                        <h4 class="text-3xl font-black text-blue-500 mb-2 italic">Rp \${x.total.toLocaleString()}</h4>
                        <p class="text-[10px] text-gray-400 font-bold uppercase mb-8">\${x.items.map(i=>i.judul).join(' • ')}</p>
                        <input id="link-\${x._id}" placeholder="MediaFire Link" class="w-full p-4 bg-black border border-blue-900 rounded-2xl text-xs mb-4 outline-none">
                        <div class="flex gap-2">
                            <button onclick="acc('\${x._id}')" class="flex-grow bg-blue-600 py-4 rounded-2xl font-black text-xs uppercase">Approve</button>
                            <button onclick="delO('\${x._id}')" class="bg-red-900/10 text-red-500 px-6 rounded-2xl font-black text-xs">X</button>
                        </div>
                    </div>
                </div>\`).join('') || '<p class="text-center py-20 text-gray-600">No orders.</p>';
            }
            ...
            async function delO(id){ if(confirm('Hapus?')){ await fetch('/api/admin/del-order/'+id,{method:'DELETE'}); location.reload(); } }
            load();
        </script>
    </body></html>`);
});

// --- 4. BACKEND API (STABLE & SECURE) ---
...
app.post('/api/admin/approve-order/:id', async (req, res) => { await Order.findByIdAndUpdate(req.params.id, { status: 'Approved', downloadLink: req.body.link }); res.json({ok:true}); });
app.delete('/api/admin/del-order/:id', async (req, res) => { await Order.findByIdAndDelete(req.params.id); res.sendStatus(200); });
