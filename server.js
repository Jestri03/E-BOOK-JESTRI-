...
app.use(cookieSession({
    name: 'jestri_session_v3',
    keys: ['SECRET_JESTRI_CORE_FINAL'],
    maxAge: 24 * 60 * 60 * 1000,
    secure: true,
    sameSite: 'none' // Diperketat untuk akses antar perangkat
}));

// --- API KATALOG (FIXED IMAGE SOURCE) ---
app.get('/api/buku', async (req, res) => {
    try {
        ...
        if (!data.length) return res.send('<div style="grid-column:1/3;text-align:center;padding:100px 20px;opacity:0.6;">Buku belum tersedia.</div>');

        res.send(data.map(b => `
            <div class="card-p">
                <div class="img-p">
                    <img src="${b.gambar}" 
                         onerror="this.onerror=null;this.src='https://placehold.co/400x600?text=Gambar+Rusak'" 
                         loading="lazy" 
                ...
                <div class="info-p">
                    ...
                    <button class="buy-p" onclick="location.href='https://wa.me/6285189415489?text=Halo+Admin,+saya+mau+order+buku+${encodeURIComponent(b.judul)}'">BELI SEKARANG</button>
                </div>
            </div>`).join(''));
...
app.get('/', async (req, res) => {
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .sidebar { position: fixed; top: 0; left: -110%; width: 280px; height: 100%; background: #fff; z-index: 200; transition: 0.4s; padding: 30px 20px; border-right: 1px solid #eee; }
        .sidebar.active { left: 0; }
        .overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.4); z-index: 150; display: none; }
        .overlay.active { display: block; }
        .g-item { display: block; width: 100%; padding: 15px; margin-bottom: 8px; border-radius: 15px; border: none; background: none; text-align: left; font-weight: 700; color: #555; cursor: pointer; font-family: inherit; }
        .g-item.active { background: #000; color: #fff; }
        .container { max-width: 800px; margin: auto; padding: 15px; }
        .search-in { width: 100%; padding: 18px; border-radius: 20px; border: 1px solid #eee; background: #f9f9f9; margin-bottom: 25px; outline: none; font-size: 1rem; }
        .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
        .card-p { background: #fff; border-radius: 20px; overflow: hidden; border: 1px solid #f2f2f2; transition: 0.3s; }
        .img-p { width: 100%; aspect-ratio: 2/3; background: #f0f0f0; position: relative; overflow: hidden; }
        ...
        .price-p { font-weight: 800; color: #2ecc71; margin: 10px 0; font-size: 1rem; }
        .buy-p { width: 100%; padding: 12px; border-radius: 12px; border: none; background: #111; color: #fff; font-weight: 800; cursor: pointer; font-size: 0.75rem; transition: 0.2s; }
        .buy-p:active { transform: scale(0.95); }
    </style></head><body>
    <div class="overlay" id="ov" onclick="tog()"></div>
    <div class="sidebar" id="sb">
        <h2 style="font-weight:800; margin-bottom: 20px;">KATEGORI</h2>
        <button class="g-item active" onclick="load('Semua', this)">Semua</button>
    ...
    <div class="nav">
        ...
        <b style="font-size: 1.2rem; letter-spacing: -1px;">E-BOOK JESTRI</b>
        <a href="https://link.dana.id/qr/39bpg786" style="background:#2ed573; color:#fff; padding:10px 18px; border-radius:50px; text-decoration:none; font-size:0.7rem; font-weight:800;">DONATE</a>
    </div>
    <div class="container">
            ...
            ${initial.map(b => `
                <div class="card-p">
                    ...
                    <div class="info-p">
                        ...
                        <h3>${b.judul}</h3>
                        <div class="price-p">Rp ${b.harga.toLocaleString('id-ID')}</div>
                        <button class="buy-p" onclick="location.href='https://wa.me/6285189415489?text=Order+${encodeURIComponent(b.judul)}'">BELI SEKARANG</button>
                    </div>
                </div>`).join('')}
        </div>
    </div>
...
app.get('/admin', async (req, res) => {
    ...
    <script>
        document.getElementById('fa').onsubmit = async (e) => {
            ...
            try {
                // UPLOAD KE IMGBB DENGAN API KEY JESTRI
                const iR = await fetch('https://api.imgbb.com/1/upload?key=63af1a12f6f91a1816c9d61d5268d948', {method:'POST', body:fd});
                const iD = await iR.json();
                const linkAsli = iD.data.url; // Ini link direct .jpg/.png
                
                await fetch('/add-ajax', { method:'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ 
                    judul: document.getElementById('j').value, penulis: document.getElementById('p').value, harga: Number(price), genre: document.getElementById('g').value, gambar: linkAsli 
                }) });
                location.reload();
