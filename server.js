...
mongoose.connect(MONGO_URI).catch(err => console.log("DB Koneksi Gagal"));
...
app.use(cookieSession({ name: 'jestri_secure', keys: ['JESTRI_MASTER_2026'], maxAge: 24 * 60 * 60 * 1000 }));
...
app.get('/admin', async (req, res) => {
    ...
    res.send(`<body style="font-family:sans-serif; background:#f4f4f4; padding:20px;"><div style="max-width:450px; margin:auto;">
        ...
        <form action="/admin/add" method="POST" style="background:#fff; padding:20px; border-radius:15px; display:grid; gap:10px;">
            ...
            <input name="gambar" placeholder="4. Link Gambar (ImgBB)" required>
            <input name="pdfUrl" placeholder="5. Link PDF (G-Drive/Mediafire)" required>
            <select name="genre">${LIST_GENRE.map(g=>`<option>${g}</option>`).join('')}</select>
            <button style="padding:15px; background:#000; color:#fff; border-radius:10px; font-weight:800; border:none;">SIMPAN BUKU</button>
        </form>
        <h3>Persetujuan Bukti TF</h3>
        ${o.map(x => `<div style="background:#fff; padding:15px; border-radius:10px; margin-top:10px; border-left:5px solid #3498db;">
            <b>${x.judulBuku}</b><br><a href="${x.bukti}" target="_blank">Lihat Bukti</a><br><br>
            <a href="/admin/approve/${x._id}" style="padding:10px; background:#2ecc71; color:#fff; text-decoration:none; border-radius:8px; font-weight:800; font-size:0.8rem;">SETUJUI</a>
        </div>`).join('')}
        <h3>Katalog Buku</h3>
        ${b.map(x => `<div style="padding:10px; background:#fff; margin-top:5px; display:flex; justify-content:space-between;">
            <span>${x.judul}</span><a href="/admin/del/${x._id}" style="color:red;">Hapus</a>
        </div>`).join('')}
    </div></body>`);
});
...
app.get('/login', (req, res) => { res.send('<form action="/login" method="POST" style="text-align:center; padding-top:100px;"><input name="pw" type="password"><button>Login</button></form>'); });
