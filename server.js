...
// Koneksi dengan opsi stabilisasi
...
app.use(cookieSession({
    name: 'jestri_session_v5',
    keys: ['SECRET_JESTRI_999'],
    maxAge: 24 * 60 * 60 * 1000
}));

// --- API DATA (OPTIMAL & ANTI-CRASH) ---
app.get('/api/buku', async (req, res) => {
    try {
        const { genre, search } = req.query;
        let filter = {};
        if (search) filter.judul = { $regex: search.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' };
        if (genre && genre !== 'Semua') filter.genre = genre;

        const data = await Buku.find(filter).sort({_id:-1}).lean().exec();
        
        if (!data || data.length === 0) {
            const emptyLabel = (genre && genre !== 'Semua') ? `Buku ${genre} belum ada` : 'Buku belum ada';
            return res.status(200).send(`<div style="grid-column:1/3;text-align:center;padding:100px 20px;opacity:0.6;font-weight:700;">${emptyLabel}</div>`);
        }

        const html = data.map(b => {
            // Proxy gambar agar tembus di semua HP (Weserv)
            const secureImg = `https://images.weserv.nl/?url=${encodeURIComponent(b.gambar.replace(/^https?:\/\//i, ''))}&noproxy=1`;
            ...
            <div class="card-p">
                <div class="img-p">
                    <img src="${secureImg}" loading="lazy" onerror="this.src='https://placehold.co/400x600?text=Foto+Buku'">
                </div>
                <div class="info-p">
                    <span class="tag-p">${b.genre}</span>
                    <h3>${b.judul}</h3>
                    <div class="price-p">Rp ${b.harga.toLocaleString('id-ID')}</div>
                    <button class="buy-p" onclick="window.location.href='https://wa.me/6285189415489?text=${waMsg}'">BELI SEKARANG</button>
                </div>
            </div>`;
        }).join('');
        ...
        res.status(200).send(`<div style="grid-column:1/3;text-align:center;padding:50px;">Gagal memuat data. Sila segarkan halaman.</div>`);
    }
});

// --- HALAMAN UTAMA (CLEAN UI) ---
    ...
    try {
        ...
        <style>
            ...
            * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
            ...
            .sidebar { position: fixed; top: 0; left: -100%; width: 280px; height: 100%; background: #fff; z-index: 200; transition: 0.3s cubic-bezier(0.4, 0, 0.2, 1); padding: 30px 20px; box-shadow: 10px 0 30px rgba(0,0,0,0.1); }
            ...
            .search-in { width: 100%; padding: 16px; border-radius: 15px; border: 1px solid #eee; background: #f9f9f9; margin-bottom: 20px; font-size: 1rem; }
            ...
            .info-p h3 { font-size: 0.85rem; margin: 5px 0; font-weight: 800; height: 2.6em; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; line-height: 1.3; }
        ...
        <div class="sidebar" id="sb">
            <h2 style="font-weight:800; margin-bottom:20px;">KATEGORI</h2>
        ...
        <div class="nav"><i class="fa-solid fa-bars" onclick="tog()" style="cursor:pointer;"></i><b>E-BOOK JESTRI</b><a href="https://link.dana.id/qr/39bpg786" style="background:#2ed573; color:#fff; padding:8px 15px; border-radius:50px; text-decoration:none; font-size:0.7rem; font-weight:800;">DONATE</a></div>
        <div class="container">
            ...
            <div class="grid" id="gt">
                ${initial.map(b => {
                    ...
                    return `<div class="card-p">
                        <div class="img-p"><img src="${secureImg}" loading="lazy"></div>
                        <div class="info-p">
                            <span class="tag-p">${b.genre}</span>
                            <h3>${b.judul}</h3>
                            <div class="price-p">Rp ${b.harga.toLocaleString('id-ID')}</div>
        ...
        res.status(500).send("Database Error. Refresh Halaman.");
    }
});

// --- ADMIN & LOGIN (STABIL) ---
app.get('/login', (req, res) => {
    res.send(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body{margin:0;background:#0f172a;height:100vh;display:grid;place-items:center;color:#fff;font-family:sans-serif;} .box{width:90%;max-width:320px;padding:30px;background:rgba(255,255,255,0.1);border-radius:20px;text-align:center;} input{width:100%;padding:12px;margin:15px 0;border-radius:10px;border:none;} button{width:100%;padding:12px;background:#3b82f6;color:#fff;border:none;border-radius:10px;width:100%;cursor:pointer;}</style></head><body><div class="box"><h2>ADMIN</h2><form action="/login" method="POST"><input type="password" name="pw" placeholder="Password" required autofocus><button>MASUK</button></form></div></body></html>`);
});
...
app.get('/admin', async (req, res) => {
    ...
    res.send(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body{font-family:sans-serif;padding:20px;background:#f4f4f4;} .card{background:#fff;padding:20px;border-radius:15px;max-width:500px;margin:auto;} input, select{width:100%;padding:10px;margin-bottom:10px;border:1px solid #ddd;border-radius:8px;} button{width:100%;padding:12px;background:#000;color:#fff;border:none;border-radius:8px;cursor:pointer;}</style></head><body>
    <div class="card">
        <div style="display:flex;justify-content:space-between;"><h3>Upload</h3><a href="/logout" style="color:red;">Logout</a></div>
        <form id="fa"><input id="j" placeholder="Judul" required><input id="p" placeholder="Penulis" required><input id="h" placeholder="Harga" type="number" required><input type="file" id="fi" required><select id="g">${LIST_GENRE.map(g => `<option>${g}</option>`).join('')}</select><button id="btn">POSTING</button></form>
        <hr><div id="list">${b.map(x => `<div style="display:flex;justify-content:space-between;padding:10px;border-bottom:1px solid #eee;"><span>${x.judul}</span><a href="/del/${x._id}" style="color:red;text-decoration:none;">Hapus</a></div>`).join('')}</div>
    </div>
    <script>
        document.getElementById('fa').onsubmit = async (e) => {
            ...
            try {
                ...
                location.reload();
            } catch(e) { alert('Gagal Upload!'); btn.disabled = false; btn.innerText = 'POSTING'; }
        };
    </script></body></html>`);
});
