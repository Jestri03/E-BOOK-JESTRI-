...
app.use(cookieSession({
    name: 'jestri_core_session',
    keys: ['ULTIMATE_JESTRI_KEY_2024'],
    maxAge: 24 * 60 * 60 * 1000
}));

// --- API DATA (DENGAN IMAGE PROXY) ---
app.get('/api/buku', async (req, res) => {
    try {
        ...
        if (!data.length) {
            ...
            return res.send(`<div style="grid-column:1/3;text-align:center;padding:120px 20px;opacity:0.6;font-weight:700;">${label}</div>`);
        }

        res.send(data.map(b => {
            // --- FIX GAMBAR: Menggunakan Proxy Weserv agar gambar tidak diblokir HP lain ---
            const cleanUrl = b.gambar.replace(/^https?:\\/\\//i, '');
            const proxiedImg = `https://images.weserv.nl/?url=${cleanUrl}&default=https://placehold.co/400x600?text=Foto+Buku&noproxy=1`;
            ...
            <div class="card-p animate-in">
                <div class="img-p">
                    <img src="${proxiedImg}" 
                         ...
                         onerror="this.src='https://placehold.co/400x600?text=Sedang+Memuat...'">
                </div>
                <div class="info-p">
...
app.get('/', async (req, res) => {
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; min-height: 400px; }
        ...
        .img-p { width: 100%; aspect-ratio: 2/3; background: #eee; overflow: hidden; }
        ...
        <div class="grid" id="gt">
            ${initial.map(b => {
                ...
                return `<div class="card-p">
                    <div class="img-p"><img src="${proxImg}" referrerpolicy="no-referrer"></div>
                    <div class="info-p">
                        <span class="tag-p">${b.genre}</span>
                        <h3>${b.judul}</h3>
                        <div class="price-p">Rp ${b.harga.toLocaleString('id-ID')}</div>
...
app.listen(PORT, () => console.log("System Perfected & Optimized"));
