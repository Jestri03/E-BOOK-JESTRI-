...
app.use(cookieSession({
    ...
    keys: ['CORE_JESTRI_FINAL_X'],
    maxAge: 24 * 60 * 60 * 1000,
    secure: true,
    sameSite: 'none'
}));

// --- API DATA (SMART RESPONSE) ---
app.get('/api/buku', async (req, res) => {
    try {
        ...
        res.send(data.map(b => {
            // FORMAT PESAN WA SESUAI REQUEST
...
app.get('/', async (req, res) => {
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .s-link { width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 1.5rem; box-shadow: 0 8px 20px rgba(0,0,0,0.15); text-decoration: none; }
        .donate-btn { background: #2ed573; color: #fff; padding: 10px 18px; border-radius: 50px; text-decoration: none; font-size: 0.75rem; font-weight: 800; }
    </style></head><body>
