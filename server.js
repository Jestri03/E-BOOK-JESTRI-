...
app.use(cookieSession({ name: 'jestri_v11', keys: ['SECRET_JESTRI_2026'], maxAge: 24 * 60 * 60 * 1000 }));

// --- 1. UI PEMBELI (FULL FEATURES & OPTIMIZED) ---
app.get('/', (req, res) => {
    ...
    <style>
        ...
        * { box-sizing: border-box; font-family: 'Plus Jakarta Sans', sans-serif; -webkit-tap-highlight-color: transparent; }
        ...
        .btn-buy { width: 100%; padding: 8px; border: none; border-radius: 8px; background: var(--d); color: white; font-weight: 700; cursor: pointer; }
    ...
    <header>
        ...
        <div class="cart-icon" onclick="openCart()"><i class="fa-solid fa-bag-shopping"></i><span id="cc" class="badge">0</span></div>
    </header>
    ...
    <div class="sosmed">
        ...
        <a href="https://www.instagram.com/jesssstri" class="btn-sos" style="background:radial-gradient(circle at 30% 107%, #fdf497 0%, #fdf497 5%, #fd5949 45%,#d6249f 60%,#285AEB 90%);"><i class="fa-brands fa-instagram"></i></a>
    </div>
    ...
    <script>
            ...
            document.getElementById('mainGrid').innerHTML = data.map(x => \`
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div style="color:var(--success); font-weight:800; font-size:0.9rem; margin-bottom:8px;">Rp \${x.harga.toLocaleString()}</div>
        ...
        function openCart(){
            ...
            document.getElementById('mContent').innerHTML = \`
                <h3 style="margin:0 0 15px;">Checkout</h3>
                \${cart.map(x=>\`<div style="font-size:0.8rem; margin-bottom:5px;">• \${x.judul}</div>\`).join('')}
                <p>Total: <b style="color:var(--success);">Rp \${cart.reduce((a,b)=>a+b.harga,0).toLocaleString()}</b></p>
                <p style="font-size:0.7rem; color:gray;">Bayar DANA: 0895327806441</p>
                <input type="file" id="fBukti" style="margin:10px 0;">
                <button onclick="checkout()" id="btnC" style="width:100%; padding:15px; background:var(--success); color:white; border:none; border-radius:12px; font-weight:bold;">BAYAR SEKARANG</button>
        ...
        async function checkout(){
            ...
            const btn = document.getElementById('btnC'); btn.innerText = "Processing..."; btn.disabled = true;
            try {
                ...
                document.getElementById('mContent').innerHTML = "<h3>Pesanan Dikirim!</h3><p>Tunggu admin cek bukti transfer lo.</p><div id='dl' style='font-size:2rem;'>⏳</div>";
                setInterval(async () => {
            ...
            } catch(e) { alert("Error saat upload. Gunakan file lebih kecil."); btn.disabled = false; }
        }
        load();
    </script></body></html>`);
});

// --- 2. ADMIN & API (OPTIMIZED & SECURE) ---
app.get('/login', (req, res) => {
    ...
    <form action="/login" method="POST" style="background:white; padding:30px; border-radius:20px; width:320px; text-align:center;">
        <h2 style="margin:0;">ADMIN JESTRI</h2>
...
app.get('/admin', async (req, res) => {
    ...
    try {
        ...
        const listO = o.map(x => `<div style="background:white; padding:15px; border-radius:12px; margin-bottom:15px; border-left:5px solid #fbbf24;">
            <b>Total: Rp ${x.total.toLocaleString()}</b><br>
            <a href="${x.bukti}" target="_blank" style="color:#38bdf8;">Bukti Transfer</a><br>
            <input type="file" id="pdf-${x._id}" style="margin:10px 0;">
            <button onclick="acc('${x._id}')" style="width:100%; padding:10px; background:#10b981; color:white; border:none; border-radius:8px;">SETUJUI</button>
            <button onclick="delO('${x._id}')" style="width:100%; padding:8px; background:#ef4444; color:white; border:none; border-radius:8px; margin-top:5px;">HAPUS</button>
        </div>`).join('');

        const listB = b.map(x => `<div style="display:flex; justify-content:space-between; padding:10px; border-bottom:1px solid #eee;"><span>${x.judul}</span><button onclick="delB('${x._id}')" style="color:red; background:none; border:none;">Hapus</button></div>`).join('');
        ...
        <h3>Order Pending</h3><div>${listO}</div>
        <h3>Katalog</h3><div class="card">${listB}</div>
        <script>
            async function addB(){
                const f = document.getElementById('fi').files[0]; if(!f) return alert("Pilih cover!");
                const btn = document.getElementById('btnS'); btn.innerText = "Uploading...";
                const fd = new FormData(); fd.append('file', f); fd.append('upload_preset', 'ml_default');
                const up = await (await fetch('https://api.cloudinary.com/v1_1/dxtp7vsqy/image/upload',{method:'POST',body:fd})).json();
...
// --- API ROUTES ---
app.post('/admin/save', async (req, res) => { try { if(req.session.admin) await new Buku(req.body).save(); res.json({ok:true}); } catch(e){ res.status(500).json({e:e.message}); } });
app.post('/admin/approve/:id', async (req, res) => { try { if(req.session.admin) await Order.findByIdAndUpdate(req.params.id, { status: 'Approved', pdfLink: req.body.pdfLink }); res.json({ok:true}); } catch(e){ res.status(500).json({e:e.message}); } });
...
app.get('/api/buku-json', async (req, res) => { try { const data = await Buku.find().sort({_id:-1}); res.json(data); } catch(e){ res.status(500).json([]); } });
app.post('/api/order', async (req, res) => { try { const o = new Order(req.body); await o.save(); res.json({id:o._id}); } catch(e){ res.status(500).json({e:e.message}); } });
app.get('/api/check/:id', async (req, res) => { try { const o = await Order.findById(req.params.id); res.json(o); } catch(e){ res.status(404).json({}); } });

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log("🚀 Server Ultron Ready!"));
